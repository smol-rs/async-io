...
fn main_loop(parker: parking::Parker) {
    let span = tracing::trace_span!("main_loop");
    ...
    loop {
        ...
        if last_tick == tick {
            ...
            if let Some(mut reactor_lock) = reactor_lock {
                tracing::trace!("main_loop: waiting on I/O");
        ...
        if BLOCK_ON_COUNT.load(Ordering::SeqCst) > 0 {
            ...
            tracing::trace!("main_loop: sleeping for {} us", delay_us);
            if parker.park_timeout(Duration::from_micros(*delay_us)) {
                tracing::trace!("main_loop: notified");
    ...
    loop {
        ...
        if let Some(mut reactor_lock) = Reactor::get().try_lock() {
            ...
            loop {
                ...
                // Wait for I/O events.
                tracing::trace!("block_on: waiting on I/O");
                reactor_lock.react(None).ok();
                ...
                if start.elapsed() > Duration::from_micros(500) {
                    tracing::trace!("block_on: stops hogging the reactor");
            ...
            tracing::trace!("block_on: sleep until notification");
            p.park();
        }
    }
}
